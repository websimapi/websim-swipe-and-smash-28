...
class Game {
    constructor() {
        ...
        this.previousRequiredOrientation = 'portrait-primary';
    ...
    async changeRequiredOrientation() {
        this.isProcessing = true;
        this.inputHandler.disable(); // Disable input during rotation

        this.previousRequiredOrientation = this.requiredOrientation;
        ...
        const oldRotation = getOrientationRotation(this.previousRequiredOrientation);
        const newRotation = getOrientationRotation(this.requiredOrientation);

        // 1. Animate candies popping out.
        const candies = Array.from(this.board.boardElement.querySelectorAll('.candy'));
        const boardHeight = this.board.boardElement.clientHeight;

        candies.forEach(candy => {
            candy.style.transition = 'transform 0.4s ease-in, opacity 0.4s ease-in';
            candy.style.transform = `translateY(-${boardHeight}px)`;
            candy.style.opacity = '0';
        });

        await new Promise(resolve => setTimeout(resolve, 400)); // wait for pop out animation

        // 2. Rotate the board container
        this.ui.gameBoardContainer.style.transform = `rotate(${newRotation}deg)`;

        // 3. Update the logical grid and prepare candy positions
        const newGrid = [];
        for (let i = 0; i < this.board.size; i++) {
            newGrid[i] = new Array(this.board.size).fill(null);
        }
        const size = this.board.size;
        const effectiveRotation = (newRotation - oldRotation + 360) % 360;
        ...
        for (let r = 0; r < size; r++) {
            for (let c = 0; c < size; c++) {
                ...
                let newR, newC;
                if (effectiveRotation === 90) { // 90 deg clockwise
                    newR = c;
                    newC = size - 1 - r;
                } else if (effectiveRotation === 180) {
                    newR = size - 1 - r;
                    newC = size - 1 - c;
                } else if (effectiveRotation === 270) { // -90 deg, or 270 clockwise
                    newR = size - 1 - c;
                    newC = r;
                } else { // no change
                    newR = r;
                ...
                newGrid[newR][newC] = candy;
                candy.dataset.row = newR;
                candy.dataset.col = newC;
            }
        }
        this.board.grid = newGrid;

        // Position candies at their new spots (still invisible and off-screen)
        const candySize = this.board.boardElement.clientWidth / this.board.size;
        candies.forEach(candy => {
            const r = parseInt(candy.dataset.row);
            const c = parseInt(candy.dataset.col);
            candy.style.transition = 'none'; // Remove transition for repositioning
        ...
        // 4. Animate candies popping back in
        candies.forEach(candy => {
            candy.style.transition = 'transform 0.4s ease-out, opacity 0.4s ease-out, top 0.3s ease, left 0.3s ease';
            candy.style.transform = 'translateY(0px)';
            candy.style.opacity = '1';
        });

        await new Promise(resolve => setTimeout(resolve, 500));

        // 5. Reset candy transitions and transforms for normal gameplay
        candies.forEach(candy => {
            candy.style.transition = 'top 0.3s ease, left 0.3s ease, transform 0.2s ease, opacity 0.3s ease';
            candy.style.transform = '';
        });

        this.inputHandler.setRotation(newRotation);
        this.checkOrientationMatch();
        this.isProcessing = false;
    }
